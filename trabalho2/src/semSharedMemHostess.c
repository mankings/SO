...
static bool checkPassport() {
    ...
    sh->fSt.st.passengerStat[sh->fSt.passengerChecked] = IN_FLIGHT;
    ...
    /* insert your code here */
    if(nPassengersInFlight() == MAXFC) last = true;
    if(nPassengersInFlight() >= MINFC && nPassengersInQueue() == 0) last = true;
    if(sh->fSt.totalPassBoarded == N) last = true;
    saveState(nFic, &sh->fSt);
...
void signalReadyToFlight() {
    ...
    sh->fSt.nPassengersInFlight[sh->fSt.nFlight] = sh->fSt.nPassInFlight;
    if(sh->fSt.totalPassBoarded == N) sh->fSt.finished = true;
    saveState(nFic, &sh->fSt);
